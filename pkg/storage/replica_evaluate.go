...
func evaluateBatch(
	...
	if baHeader.Txn != nil {
		...
		// applies to writes that leave intents and reads that occur in a
		// transaction that has already written (see #2231 for more about why we
		// check for aborted transactions on reads). Note that 1PC transactions
		// have had their transaction field cleared by this point so we do not
		// execute this check in that case.
		if baHeader.Txn.IsWriting() {
