...
type requestSequencer interface {
	// SequenceReq acquires latches, checks for locks, and queues and/or pushes
	// other transactions to resolve any conflicts. Once sequenced, the request
	// is guaranteed sufficient isolation for the duration of evaluation.
	...
	//
	// TODO: it should be ok to return a response here, if necessary
	SequenceReq(context.Context, *Guard /* optional */, Request) (*Guard, *Error)
	...
	// requests that are blocked on this one to proceed.
	FinishReq(*Guard)
}

// contentionHandler is concerned with TODO
...
type rangeUpdateListener interface {
	...
	OnLeaseUpdated(iAmTheLeaseHolder bool)
...
type lockTable interface {
	// NB: don't add replicated if not already exist.
	acquireLock( /* TODO */ )
	releaseLock( /* TODO */ )
	addDiscoveredLock(Request, roachpb.Intent) lockWaitQueueGuard
	scanAndEnqueue(Request) []lockWaitQueueGuard
...
type txnWaitQueue interface {
	...
	GetDependents(uuid.UUID) []uuid.UUID
	MaybeWaitForPush(context.Context, *roachpb.PushTxnRequest) *Error
	MaybeWaitForQuery(context.Context, *roachpb.QueryTxnRequest) *Error
}

// TODO LIST:
// X 1. don't mutate request on txnwait.ErrDeadlock
// X 2. don't return BatchResponse from maybeWaitForPushee
// X 3. delete IsLeaseRequest
// X 4. merge MaybeInterceptReq into SequenceReq, delete requestInterceptor
// ? 5. create queue in HandleWriterIntentError but don't wait, make sure GC is clean
// 7. begin blocking on lockWaitQueues in SequenceReq
// 8. hook up lock updates to lockWaitQueue like we do for txnWaitQueues.
