...
// bounds. The function will return either a batch response or an error. If a
// batch response is returned, it may also return a flag indicating that the
// request is continuing to execute asynchronously and that it has assumed
// responsibility of releasing the guard.
type batchExecutionFn func(
	*Replica, context.Context, *roachpb.BatchRequest, *spanset.SpanSet, *concurrency.Guard,
) (_ *roachpb.BatchResponse, async bool, _ *roachpb.Error)
...
func (r *Replica) executeBatchWithConcurrencyRetries(
	...
	for {
		...
		g, pErr = r.concMgr.SequenceReq(ctx, g, concurrency.Request{
			Txn:             ba.Txn,
		...
		var async bool
		br, async, pErr = fn(r, ctx, ba, spans, g)
		switch t := pErr.GetDetail().(type) {
		case nil:
			// Success.
			if async {
				// If the request is continuing to execute asynchronously, it
				// has assumed responsibility for releasing the concurrency
				// guard when it finishes.
				g = nil
			}
			return br, nil
		case *roachpb.WriteIntentError:
			// Drop latches, but retains lock wait-queues.
...
func (r *Replica) handleWriteIntentError(
	...
	g, newPErr := r.concMgr.HandleWriterIntentError(ctx, g, t)
	if newPErr != nil {
