...
type blockingBuffer struct {
	...
	// HACK to avoid allocating while fitting the quotapool.Request interface.
	// This is bad, but there are no concurrent calls to blockingBuffer.Add in
	// practice.
	// The thread safety of this buffer should be considered. Does it need to
	// be multi-producer, multi-consumer? We're paying for it.
	tmpEvent Event
...
func (b *blockingBuffer) Add(ctx context.Context, e Event) error {
	...
	if err := b.qp.Acquire(ctx, (*bufferEntry)(r)); err != nil {
		return err
	}
