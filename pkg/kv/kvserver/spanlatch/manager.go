...
func (m *Manager) iterAndWait(
	...
	t *timeutil.Timer,
	it *iterator,
	waitAccess, heldAccess spanset.SpanAccess,
	wait *latch,
	ignore ignoreFn,
) error {
	for it.FirstOverlap(wait); it.Valid(); it.NextOverlap(wait) {
		...
		if err := m.waitForSignal(ctx, t, waitAccess, heldAccess, wait, held); err != nil {
			return err
		}
	}
	return nil
...
func (m *Manager) waitForSignal(
	...
	log.Eventf(ctx, "waiting to acquire %s latch %s, held by %s latch %s", waitAccess, wait, heldAccess, held)
	for {
		select {
