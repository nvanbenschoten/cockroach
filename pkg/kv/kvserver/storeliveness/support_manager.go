...
func (sm *SupportManager) onRestart(ctx context.Context) error {
	...
	if err := sm.clock.UpdateAndCheckMaxOffset(ctx, sm.ss.meta.MaxWithdrawn); err != nil {
		return err
	}
...
func (sm *SupportManager) handleHeartbeat(
	...
	log.VInfof(ctx, 2, "handled heartbeat from %+v with epoch %+v and expiration %+v, ack: %t",
		msg.From, msg.Epoch, msg.Expiration, ack)
	resp = slpb.Message{
...
func (sm *SupportManager) handleHeartbeatResp(
	...
	log.VInfof(ctx, 2, "handled heartbeat response from %s with expiration %s",
		msg.From, msg.Expiration)
}
