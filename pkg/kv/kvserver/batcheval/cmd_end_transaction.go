...
func EndTxn(
	...
	if args.Commit {
		...
		switch {
		case !recordAlreadyExisted,
			existingTxn.Status == roachpb.PENDING,
			// WIP: is this legit? What about the deadline?
			existingTxn.Status == roachpb.PREPARED && existingTxn.IsoLevel.ToleratesWriteSkew():
			// Check timestamp cache. The transaction could have been pushed.
			BumpToMinTxnCommitTS(ctx, cArgs.EvalCtx, reply.Txn)
		case existingTxn.Status == roachpb.PREPARED && !existingTxn.IsoLevel.ToleratesWriteSkew():
			// Don't check timestamp cache. The transaction could not have been pushed
			// while its record was in the PREPARED state because it is Serializable.
			// Furthermore, checking the timestamp cache and increasing the commit
			// timestamp at this point would be incorrect, because the transaction
			// must not fail to commit after being prepared.
	...
	// TODO: remove args from these functions.
